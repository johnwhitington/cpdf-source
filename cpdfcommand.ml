...
let version_date = "(29th May 2026)"
...
let args =
  {op = None;
   ...
   outline = false;
   linewidth = 1.0;
   path_to_ghostscript = begin match Sys.os_type with "Unix" -> "gs" | _ -> match Sys.word_size with 64 -> "gswin64c.exe" | _ -> "gswin32c.exe" end;
...
let rec go () =
  ...
  | ListImages ->
      ...
      let json = Cpdfimage.images ~inline:args.inline pdf range in
        if args.format_json then
          ...
          | `List l ->
              iter
                (function (`Assoc [(_, `Int i);
                                   (_, `List pages);
                                   (_, `String name);
                                   (_, `Int w); (_, `Int h);
                                   (_, `Int size);
                                   (_, bpc); (_, cs); (_, filter);
                                   (_, `String masktype); (_, maskobjnum)]) ->
                   ...
                   let maskobjnum = match maskobjnum with `Int n -> string_of_int n | _ -> "none" in
                     flprint (Printf.sprintf "%i, %s, %s, %i, %i, %i, %s, %s, %s, %s, %s\n" i pages name w h size bpc cs filter masktype maskobjnum)
                 | _ -> ())
                l
          | _ -> ()
          end
  ...
  | RedactApply ->
      let pdf = get_single_pdf args.op false in
      let range = parse_pagespec pdf (get_pagespec ()) in
      let pdf = Cpdfredact.apply pdf ~color:args.color ~outline:args.outline ~opacity:args.opacity ~linewidth:args.linewidth ~underneath:args.underneath range in
        write_pdf false pdf
  | RedactApplyType s ->
      let pdf = get_single_pdf args.op false in
      let range = parse_pagespec pdf (get_pagespec ()) in
