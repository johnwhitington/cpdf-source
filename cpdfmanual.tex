...
Annotations denoting future redactions can be added to a PDF in Adobe Acrobat other interactive PDF software. Cpdf can apply these annotations to perform the redaction. Cpdf can also create such redaction annotations. To apply existing annotation redactions:
  ...
  \begin{framed}
    ...
    \vspace{2.5mm}
    \noindent Apply redaction annoations in \texttt{in.pdf}, writing to \texttt{out.pdf}
  \end{framed}
...
\begin{description}
\item[Inversion] Adding \texttt{-redact-invert} and writing to another PDF will keep all objects redaction would lose, and lose all objects redaction would keep. Each object (image, vector, glyph) ends up only the redacted or inverted output (note that images are never chopped with \texttt{-redact-invert}).
